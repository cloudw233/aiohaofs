...
class Account:
    ...
    session: ClientSession = None # aiohttp会话
    ...
    async def login(self, email, password):
        ...
        self.logged_in = r_code == 200
        self.logger.info(f'邮箱：{email} 登录结果：{r_code}')
    ...
    async def register(self,
        ...
        temp_mail = TempMail('aiohfs' + xuehao + generate_random_string(6),
                             password)
    ...
    async def __get_account_data(self):
    ...
    def __str__(self):
        ...
        '''
        if self.logged_in:
            return f'{asyncio.run(self.student)["xuehao"]}-{asyncio.run(self.student)["studentName"]}'
        else:
            return '未登录'
...
class Exam(IncludeAccount):
    ...
    def __init__(self, account: Account, examId: int):
        ...
        async with self.session as session:
            async with session.get(
            f'https://hfs-be.yunxiao.com/v3/exam/{examId}/overview') as resp:
                r = asyncio.run(resp.json())
...
class Paper(IncludeAccount):
    ...
    async def save_pictures(self, path: str):
        '''
            保存该试卷所有答题卡的图片到指定路径。
            @param path: 文件保存路径
            文件命名格式：
        '''
        for i in range(len(await self.pictures)):
            self.logger.info(f'正在保存图片[{self.paperData["name"]}-{i+1}.png]...')
            async with self.session as session:
                async with session.get((await self.pictures)[i]) as resp:
                    r = await resp
